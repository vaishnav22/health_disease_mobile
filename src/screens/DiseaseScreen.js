...
const DiseaseScreen = ({navigation}) => {
    ...
    return (
        ...
        <View style={styles.pickerView}>
          ...
          <View style={styles.pickerView}>
          <Picker style={styles.picker} selectedValue={symptom6} onValueChange={(itemValue) => setSymptom6(itemValue)}>
              {symptoms.map((item,index) => (
                <Picker.Item value={item} label={item} key={index}/>
              ))}
          </Picker>
          ...
          <FormButton
            buttonTitle="Predict"
            onPress={() => predict(symptom1, symptom2, symptom3, symptom4)}
          />
